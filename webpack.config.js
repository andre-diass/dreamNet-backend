...
(function () {
  ...
  const allowedStages = ['lcl', 'dev', 'prod'];
  // eslint-disable-next-line prefer-destructuring
  const stage = slsw.lib.serverless.service.provider.stage;
  console.log('\n## ALLOWED STAGES ##');
  console.log(allowedStages);
  if (allowedStages.includes(stage) === false) throw new Error(`Stage "${stage}" not allowed`);
  else console.log(`STAGE=${stage}`);
  console.log('------------------------------------------\n');
})();
    ...
    rules: [
      {
        test: /\.ts$/,
        loader: 'ts-loader',
        exclude: [[path.resolve(__dirname, '.webpack'), path.resolve(__dirname, '.serverless'), /node_modules/]],
